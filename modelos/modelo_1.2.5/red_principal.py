...
def cargar_datos_especificos(archivo_potencias='potencias.csv', archivo_corrientes='corrientes.csv', dias_semanales=None, horas=None):
    ...
    try:
        ...
        df_unido['numero_de_medicion'] = range(1, len(df_unido) + 1)

        # Seleccionar y reorganizar las columnas en el formato deseado
        final_df = df_unido[['activa', 'dia_sen', 'dia_cos', 'mes_sen', 'mes_cos', 'diferencia_activa', 'numero_de_medicion']]
...
def entrenar_modelo(Xtrain, ytrain, Xval, yval, path_guardado='modelo_entrenado.h5'):
    ...
    model.add(LSTM(340, return_sequences=True, input_shape=(Xtrain.shape[1], Xtrain.shape[2]), kernel_initializer=initializer ) )
    #model.add(Dropout(0.1))
    model.add(BatchNormalization())

    #model.add(Dense(128, activation='relu', kernel_initializer=initializer))
    #model.add(Dropout(0.2)) 
    model.add(LSTM(148, return_sequences=False, kernel_initializer=initializer ) )
    #model.add(Dropout(0.1))
    #model.add(BatchNormalization())
